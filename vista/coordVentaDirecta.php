...
<?php 
    ...
    } else if($_SESSION['rol'] == "coord_financiera" || $_SESSION['rol'] == "coord_venta_directa" || $_SESSION['rol'] == "lider" || $_SESSION['rol'] == "asesor"){
        header("location:acceso_denegado.php");
    }
...
?>
    ...
    <div class="container-fluid">
        ...
        <form action="" method="post">
            <button type="submit" name="botonRegistrar" class="mt-3 mb-3 btn btn-primary font-weight-bold" data-toggle="modal" data-target="#form_coordVentaDirecta1"><i class="fas fa-plus"></i> REGISTRAR COORDINADOR DE VENTA DIRECTA</button>
        </form>
        
        <!-- Lista de coordinadores de venta directa -->
        <form action="" method="post">
        <table id="tablaDinamica" class="table table-striped table-responsive-xl scroll_modificado">
            ...
            <tbody class="table-light">
                    <?php
                        ...
                        $objetoAD = new coordVentaDirectaDao();
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <button type="submit" name="botonModificar" class="btn btn-success" value="<?php echo $rowA[0]?>" data-toggle="modal" data-target="#form_coordVentaDirecta2"><i class="fas fa-pencil-alt"></i></button>
                        </td>
                    </tr>
                    <?php endforeach; ?>
            </tbody>
        ...
        <?php
            include("modal/mRegistrarCoordVentaDirecta.php"); // Modal Registrar
            if(isset($_POST['botonModificar'])): // traer informacón de item seleccionado
                $IdbotonModificar = $_POST['botonModificar'];
                $objetoA = new coordVentaDirectaDao(); 
                $listarCoordVentaDirecta = $objetoA->listarItem($IdbotonModificar);
                
                foreach($listarCoordVentaDirecta as $rowAna):
                    include("modal/mModificarCoordVentaDirecta.php"); // Modal Modificar 
                endforeach;
            endif;
        ?>
    </div>
    ...
    <?php 
        ...
        if(isset($_POST['botonModificar'])){
            echo "<script>$('#form_coordVentaDirecta2').modal('show');</script>";
        }

        // Abrir modal Registrar si se dió clic en boton registrar
        if(isset($_POST['botonRegistrar'])){
            echo "<script>$('#form_coordVentaDirecta1').modal('show');</script>";
        }
    ?>
